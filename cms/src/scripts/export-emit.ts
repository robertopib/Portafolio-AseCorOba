/**
 * Export + FIDELITY GATE — the reconstruction itself (Local API twin).
 *
 ...
 *
 * Run it through its CLI:  pnpm payload run src/scripts/export-content.ts
 *
 ...
 *
 * ── Why this is a separate module from export-content.ts (R13b) ──────────────
 * `payload` and `@payload-config` are imported by the CLI, NOT here, and this
 * module auto-runs nothing. That is what lets the root test suite import it and
 * drive the reconstruction over a fixture with no Payload boot, no database and
 * no network — the `tests` CI job installs root dependencies only, so anything
 * reaching `payload` could not even resolve there. Keep this module's imports to
 * node builtins and ./content-map.
 *
 * The gate itself (exit codes, FIDELITY_GATE, the error file) lives in the CLI.
 */
import fs from 'fs'
...
export const OUT_DIR = '/tmp/export-out'
const readJson = (p: string) => JSON.parse(fs.readFileSync(p, 'utf-8'))

export type FileReport = { match: boolean | null; diffs?: string[]; note?: string }

/**
 * The single seam this twin reads through — the object `getPayload({ config })`
 * returns, narrowed to the two methods the reconstruction actually calls. The
 * CLI supplies the real Payload instance; a test supplies a fake.
 *
 * `import type` is erased at compile time, so this keeps Payload's real types on
 * the CMS side without putting `payload` in the module's RUNTIME import graph —
 * which is the whole point of this file (see the header).
 */
export type PayloadReader = Pick<import('payload').BasePayload, 'find' | 'findGlobal'>

export interface ExportOptions {
  /** The data source. Required — this module never boots Payload itself. */
  payload: PayloadReader
  /** Where to write the reconstruction. `null` = don't write anything. */
  outDir?: string | null
  /** Committed content to diff against. `null` = skip the comparison. */
...
export interface ExportResult {
   ...
   * + '\n'`). THIS is what the two twins must agree on: a key valued `undefined`
   * is present in `written` but dropped from the text.
   */
  serialized: Record<string, string>
  report: Record<string, FileReport>
  summary: ReturnType<typeof summarizeFidelity>
}
...
// reconstruction logic itself. Edit both sides in the same commit.
//
// R13b looked at collapsing it and deliberately did not: every option (vendoring
// with a generated-file check, a published internal module, a build step for
// cms/) puts a code generator between this file and `next build`, and a broken
// CMS deploy is far worse than a duplicated 60-line helper. What R13b DID do is
// make the mirror verifiable instead of merely asserted — these three functions
// are exported so tests/fidelity/fidelity-gate.test.ts can drive them and the
// originals in scripts/lib/fidelity.mjs over the same inputs and require
// identical results. Edit one side without the other and that test goes red.
 ...
 */
export function summarizeFidelity(files: Record<string, FileReport>, expected: string[] = []) {
...
export function formatFidelityFailure(
  ...
  out.push(
    ...
    // Verbatim from scripts/lib/fidelity.mjs. It read "the CMS reconstruction
    // and…" until R13b: the two copies were written in the same commit (R13a,
    // 5830a00) and were never actually identical. Nothing detected that, which is
    // precisely the argument for the equivalence test that now does.
...
export function deepDiff(a: any, b: any, pathStr = ''): string[] {
  const diffs: string[] = []
  const ta = typeof a
  const tb = typeof b
...
//
// Module scope, not a closure inside main(), for one reason: it is the whole
// behavioural change and it is pure, so `tests/unit/r23-flatten.test.ts` can
// pin the three group modes and the sort directly, with no Payload, no
// database and no twin harness. That is the shape reconcile.ts settled on in
// R49 and the roadmap asked to carry forward.
//
// It is still HAND-MIRRORED into scripts/fetch-content.mjs rather than shared.
// cms/ is a separate pnpm project with its own lockfile and its own Vercel root
// directory, so a `../../../scripts/` import does not resolve in the CMS
// deployment — the reasoning is spelled out in scripts/lib/fidelity.mjs's
// header. tests/fidelity/twin-equivalence.test.ts is what keeps the copies
// honest: it compares the two emitters AS BYTES.
// ===========================================================================

/** One photograph, with the parent it hangs off and its position in that parent. */
export type FlatImage = { img: any; parent: any; index: number }
 ...
 * committed byte.
 */
export const sortFlat = (rows: FlatImage[], home: boolean): FlatImage[] =>
  [...rows].sort(
    (a, b) =>
...
/**
 * Reconstruct every content file from the CMS and diff it against committed
 * content. The DB guard, the Payload boot, the exit code and the error file all
 * live in the CLI (export-content.ts) — this function has no process side
 * effects, which is what makes it testable.
 *
 * Defaults reproduce the CLI exactly; `outDir: null` / `compareDir: null` are
 * for R13b's twin-equivalence test, which compares the two EMITTERS against each
 * other rather than either of them against committed content.
 */
export async function main({
  payload,
  outDir = OUT_DIR,
  compareDir = CONTENT_DIR,
  ...
  if (outDir) fs.mkdirSync(path.join(outDir, 'sections'), { recursive: true })
  ...
  const emit = (relPath: string, recon: any, origPath: string) => {
    ...
    if (outDir) fs.writeFileSync(path.join(outDir, relPath), text)
    if (!compareDir) return
    // Every origPath a caller passes is under CONTENT_DIR, so this is identity
    // for the CLI and redirects the whole comparison when compareDir is moved.
    const cmpPath = path.join(compareDir, path.relative(CONTENT_DIR, origPath))
    if (!fs.existsSync(cmpPath)) {
      report[relPath] = { match: null, note: `no committed ${path.relative(CONTENT_DIR, origPath)} to diff against` }
      return
    }
    const diffs = deepDiff(recon, readJson(cmpPath))
  ...
  // twin-equivalence.test.ts compares what they emit AS BYTES. Change one side
  // only and that test goes red — correctly.
  // =========================================================================

  /** Bind the module-scope flattener to this run's projects. */
  const flatten = (opts: { slug: string; group?: string | null; home: boolean }) =>
    flattenImages(allProjects.docs as any[], slugOf, opts)
   ...
   * change.
   */
  const galleryCard = ({ img, parent }: FlatImage, asHome: boolean) => {
    const card: any = {
  ...
  const resolveGalleryCards = (opts: {
    ...
    const clip = (rows: FlatImage[]) => (maxItems && maxItems > 0 ? rows.slice(0, maxItems) : rows)
  ...
  // (R13a). It is now the script's exit code, decided by the CLI from what this
  // returns.
  const summary = summarizeFidelity(report, Object.keys(written))
  return { written, serialized, report, summary }
}
