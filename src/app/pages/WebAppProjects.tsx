...
export function WebAppProjects() {
  ...
  const nextImage = () => {
    if (selectedImage !== null) {
      setSelectedImage((selectedImage + 1) % webAppProjects.length);
    }
  };

  const prevImage = () => {
    if (selectedImage !== null) {
      setSelectedImage((selectedImage - 1 + webAppProjects.length) % webAppProjects.length);
    }
  };
        ...
        {selectedImage !== null && (
          ...
          >
            ...
            <div className="flex flex-col items-center justify-center flex-1 w-full" onClick={(e) => e.stopPropagation()}>
              <img
                src={webAppProjects[selectedImage].src}
                alt={webAppProjects[selectedImage].alt}
                className="max-w-full max-h-[calc(100vh-140px)] w-auto h-auto object-contain rounded-[3px]"
              />
              <div className="text-center mt-6">
                <p className="text-neutral-100 text-lg">{webAppProjects[selectedImage].alt}</p>
                <p className="text-neutral-100/60 text-sm mt-2">
                  {selectedImage + 1} / {webAppProjects.length}
                </p>
              </div>
            </div>
          </div>
